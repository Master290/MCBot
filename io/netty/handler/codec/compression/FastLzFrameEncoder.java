  ...
  extends MessageToByteEncoder<ByteBuf>
{
  ...
  public FastLzFrameEncoder(int level, Checksum checksum)
  {
    if ((level != 0) && (level != 1) && (level != 2)) {
      throw new IllegalArgumentException(String.format("level: %d (expected: %d or %d or %d)", new Object[] {
        Integer.valueOf(level), Integer.valueOf(0), Integer.valueOf(1), Integer.valueOf(2) }));
    }
    this.level = level;
  ...
  protected void encode(ChannelHandlerContext ctx, ByteBuf in, ByteBuf out) throws Exception
  {
    ...
    for (;;)
    {
      ...
      if (length < 32) {
        ...
        int compressedLength = FastLz.compress(in, in.readerIndex(), length, out, outputPtr, level);
